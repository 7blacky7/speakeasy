...
fn session_aus_metadata(
    ...
    token_validator(token)
        .map_err(|_| Status::unauthenticated("Ungueltiger oder abgelaufener Token"))
}
        ...
        match self.state.ausfuehren(Command::KanalListe, session).await {
            Ok(crate::commands::types::Response::KanalListe(kanaele)) => {
                let channels: Vec<ChannelInfo> =
                    kanaele.into_iter().map(kanal_info_zu_proto).collect();
        ...
        match self.state.ausfuehren(cmd, session).await {
            Ok(crate::commands::types::Response::Kanal(kanal)) => {
                Ok(Response::new(CreateChannelResponse {
                    channel: Some(kanal_info_zu_proto(kanal)),
                }))
            }
            Ok(_) => Err(Status::internal("Unerwarteter Response-Typ")),
            Err(e) => Err(commander_error_zu_status(e)),
...
impl proto::client_service_server::ClientService for ClientServiceImpl {
    async fn list_clients(
        ...
        match self.state.ausfuehren(Command::ClientListe, session).await {
            Ok(crate::commands::types::Response::ClientListe(clients)) => {
                let proto_clients: Vec<ClientInfo> =
                    clients.into_iter().map(client_info_zu_proto).collect();
    ...
    async fn get_client(&self, _request: Request<UserId>) -> Result<Response<ClientInfo>, Status> {
...
impl proto::permission_service_server::PermissionService for PermissionServiceImpl {
    async fn get_permissions(
        ...
        match self
            .state
            .ausfuehren(
                Command::BerechtigungListe {
                    ziel: body.target.clone(),
    ...
    async fn set_permission(
        ...
        let wert = body
            .value
            .map(proto_zu_berechtigung)
            .unwrap_or(BerechtigungsWertInput::Skip);
        self.state
            .ausfuehren(
...
impl proto::file_service_server::FileService for FileServiceImpl {
                ...
                let files: Vec<FileEntry> = dateien
                    ...
                    .map(|d| FileEntry {
                        ...
                        channel_id: Some(ChannelId {
                            value: d.kanal_id.to_string(),
                        }),
                        uploaded_by: Some(UserId {
                            value: d.hochgeladen_von.to_string(),
                        }),
    ...
    async fn delete_file(
        ...
        self.state
            .ausfuehren(
                Command::DateiLoeschen {
                    datei_id: body.file_id,
                },
                session,
            )
            .await
...
fn kanal_info_zu_proto(k: crate::commands::types::KanalInfo) -> ChannelInfo {
    ChannelInfo {
        ...
        parent_id: k.parent_id.map(|id| ChannelId {
            value: id.to_string(),
        }),
...
fn client_info_zu_proto(c: crate::commands::types::ClientInfo) -> ClientInfo {
    ClientInfo {
        user_id: Some(UserId {
            value: c.user_id.to_string(),
        }),
        username: c.username.clone(),
        display_name: c.username,
        channel_id: c.kanal_id.map(|id| ChannelId {
            value: id.to_string(),
        }),
        server_groups: vec![],
