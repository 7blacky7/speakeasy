...
pub async fn list_files(State(state): State<CommanderState>, Path(kanal_id): Path<Uuid>, headers: HeaderMap) -> Response {
    let session = match session_aus_headers(&headers, &state) { Ok(s) => s, Err(r) => return r };
    match state.ausfuehren(Command::DateiListe { kanal_id }, session).await {
...
pub async fn delete_file(State(state): State<CommanderState>, Path(datei_id): Path<String>, headers: HeaderMap) -> Response {
    let session = match session_aus_headers(&headers, &state) { Ok(s) => s, Err(r) => return r };
    match state.ausfuehren(Command::DateiLoeschen { datei_id }, session).await {
