...
pub async fn put_server(State(state): State<CommanderState>, headers: HeaderMap, Json(body): Json<ServerBearbeitenBody>) -> Response {
    let session = match session_aus_headers(&headers, &state) { Ok(s) => s, Err(r) => return r };
    let cmd = Command::ServerEdit { name: body.name, willkommensnachricht: body.willkommensnachricht, max_clients: body.max_clients, host_nachricht: body.host_nachricht };
...
#[derive(Debug, Deserialize)]
pub struct ServerStoppenBody { pub grund: Option<String> }

pub async fn post_server_stop(State(state): State<CommanderState>, headers: HeaderMap, Json(body): Json<ServerStoppenBody>) -> Response {
    let session = match session_aus_headers(&headers, &state) { Ok(s) => s, Err(r) => return r };
    match state.ausfuehren(Command::ServerStop { grund: body.grund }, session).await {
